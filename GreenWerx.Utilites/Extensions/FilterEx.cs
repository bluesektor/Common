...
namespace GreenWerx.Utilites.Extensions
{
    public static class FilterEx
    {
        public static List<dynamic> DistinctBy(this List<dynamic> input, DataScreen screen)
        {
            ...
            switch (screen.Field?.ToUpper())
            {
                ...
                case "CATEGORYUUID":
                    ...
                    #endregion original code not ported. may not be needed
        ...
        public static List<dynamic> Filter(this List<dynamic> input, ref DataFilter filter)//, out int resultCount )
        {
            // resultCount = 0;
            ...
            switch (screen.Field.ToUpper())
            {
                ...
                case "DATECREATED":
                    if (orderbyDesc)
                        ...
                        input = input.OrderBy(w => w.DateCreated).ToList();
                    break;

                case "STARTDATE":
                    if (orderbyDesc)
                        input = input.OrderByDescending(d => d.StartDate).ToList();
                    else
                        input = input.OrderBy(w => w.StartDate).ToList();
                    break;
                //Strain Specific
                case "HARVESTTIME":
                    if (orderbyDesc)
        ...
        /// <returns></returns>
        public static List<dynamic> Search(this List<dynamic> input, ref DataFilter filter, out int resultCount)
        {
            resultCount = 0;
            List<dynamic> results = new List<dynamic>();
                ...
                switch (screen.Field?.ToUpper())
                {
                    case "BODY":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.Body?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.Body?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.Body?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                   ...
                   case "REFERENCEUUID":
                    ...
                    case "LOOKINGFOR":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.LookingFor?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.LookingFor?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.LookingFor?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "TYPE":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.Type?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.TYpe?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.Type?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "SENDTO":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.SendTo?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.SendTo?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.SendTo?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                    ...
                    case "SENDFROM":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.SendFrom?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.SendFrom?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.SendFrom?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "DATESENT":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.DateSent?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.DateSent?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.DateSent?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "DATEOPENED":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.DateOpened?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.DateOpened?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.DateOpened?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "IPADDRESS":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.IpAddress?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.IpAddress?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.EmailTo?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;

                    case "EMAILTO":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.EmailTo?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.EmailTo?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.EmailTo?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                    ...
                    case "EMAILFROM":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.EmailFrom?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.EmailFrom?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.EmailFrom?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "NAMETO":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.NameTo?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.NameTo?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.NameTo?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "NAMEFROM":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.NameFrom?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.NameFrom?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.NameFrom?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "USERUUID":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.UserUUID?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.UserUUID?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.UserUUID?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "SUBJECT":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.Subject?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.Subject?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.Subject?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    ...
                    case "MESSAGE":
                        if (screen.Operator.EqualsIgnoreCase("CONTAINS"))
                            input = input.Where(w => w.Message?.ToUpper()?.Contains(screen.Value.ToUpper().ToUpper())).ToList();
                        else if (screen.Operator.EqualsIgnoreCase("SEARCH!BY"))
                            input = input.Where(w => w.Message?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) == -1).ToList();
                        else
                            input = input.Where(w => w.Message?.IndexOf(screen.Value, StringComparison.OrdinalIgnoreCase) != -1).ToList();
                        break;
                    //API Key filter
