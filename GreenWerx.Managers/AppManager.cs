    ...
    public class AppManager : BaseManager
    {
        ...
        public async Task<ServiceResult> BackupDatabase(string encryptionKey)
        {
            ...
            try
            {
                ...
                pathToTempBackupFile = Path.Combine(pathToBackupFolder, "GreenWerx." + Guid.NewGuid().ToString("N") + ".tmp");

                FileInfo file = new FileInfo(pathToScript);
                string script = file.OpenText().ReadToEnd();
                script = script.Replace("{{DATABASE_NAME}}", "GreenWerx");
                ...
                string pathToBackupFile = Path.Combine(pathToBackupFolder, "GreenWerx." + string.Format("text-{0:yyyy-MM-dd_hh-mm-ss-tt}", DateTime.UtcNow) + ".bak");
        ...
        public string CreateConnectionString(AppInfo appSettings)
        {
            ...
            switch (appSettings.ActiveDbProvider?.ToUpper())
            {
                ...
                case "SQLITE":
                    string pathToDb = Path.Combine(EnvironmentEx.AppDataFolder, "GreenWerx.sqlite");
        ...
        {
            GreenWerxDbContext appData = new GreenWerxDbContext(true);
        ...
        {
            string database = new GreenWerxDbContext(this._connectionKey).Database.Connection.Database;
            return database;
        }
        ...
        public Setting GetSetting(string name )//, bool useDatabase = false)
        {
            ...
            try
            {
                //if (useDatabase) // todo reimplement later
                //{
                //    using (var context = new GreenWerxDbContext(this._connectionKey))
                //    {
            ...
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                switch (n.UUIDType?.ToUpper())
                {
                    case "ACCOUNT":
                        ...
                        var react = accountManager.Get(n.UUID);
                        if (react.Code != 200)
                            return react;
                        Account a = (Account)react.Result;
                    ...
                    case "ANATOMY":
                        ...
                        var resa = AnatomyManager.Get(n.UUID);
                        if (resa.Code != 200)
                            return resa;

                        Anatomy anatomy = (Anatomy)resa.Result;
                    ...
                    case "CATEGORY":
                        ...
                        var resc = categoryManager.Get(n.UUID);
                        if (resc.Code != 200)
                            return resc;
                        Category category = (Category)resc.Result;
                        if (category == null)
                    ...
                    case "CURRENCY":
                        ...
                        var rec = financeManager.Get(n.UUID);
                        if (rec.Code != 200)
                            return rec;
                        Currency currency = (Currency)rec.Result;
                    ...
                    case "PRODUCT":
                        ...
                        var resp = productManager.Get(n.UUID);
                        if (resp.Code != 200)
                            return resp;
                        Product product = (Product)resp.Result;
                        if (product == null)
                    ...
                    case "STRAIN":
                        ...
                        var ress = strainManager.Get(n.UUID);
                        if (ress.Code != 200)
                            return ress;

                        Strain strain = (Strain)ress.Result;
                    ...
                    case "VENDOR":
                        ...
                        var resi = VendorManager.Get(n.UUID);
                        if (resi.Code != 200)
                            return resi;
                        Vendor Vendor = (Vendor)resi.Result;
                        if (Vendor == null)
                            return ServiceResponse.Error("Product was not found.");
        ...
        public async Task<ServiceResult> RestoreDatabase(string fileName, string encryptionKey)
        {
            ...
            try
            {
                ...
                pathToDecryptedFile = Path.Combine(pathToBackupFolder, "GreenWerx." + Guid.NewGuid().ToString("N") + ".tmp");
        ...
        public ServiceResult SeedDatabase(string pathToFolder)
        {
            ...
            foreach (string table in tables)
            {
                ...
                ////        dataItem.RoleWeight =  RoleFlags.MemberRoleWeights.Member;
            ...
            switch (tableObject.GetType().Name.ToUpper())
            {
                ...
                case "CURRENCY":
                    ...
                    using (var context = new GreenWerxDbContext(this._connectionKey))
                    {
                        ...
                        {
                            //If the sync key doesn't exist as uuid then it hasn't been imported during the install.
                            if (context.GetAll<Currency>(ref filter)?.FirstOrDefault(x => x.UUID == currency.GUUID) == null
                                   && string.IsNullOrWhiteSpace(currency.GUUID) == false)
                                currency.UUID = currency.GUUID;

                            if (validate &&
                                 context.GetAll<Currency>(ref filter)?.FirstOrDefault(x => x.Name.EqualsIgnoreCase(currency.Name) && x.AccountUUID == accountUUID) != null)
                            {
                                msg.AppendLine("Currency already exists:" + currency.Name);
                                continue;
                            }
                            else if (validateGlobally &&
                                    context.GetAll<Currency>(ref filter)?.FirstOrDefault(x => x.Name.EqualsIgnoreCase(currency.Name)) != null)
                            {
