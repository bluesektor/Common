    ...
    public class FinanceAccountTransactionsManager : BaseManager, ICrud
    {
        ...
        public ServiceResult Get(string uuid)
        {
            ...
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                ...
                var res = context.GetAll<FinanceAccountTransaction>()?.FirstOrDefault(sw => sw.UUID == uuid);
                if (res == null)
                    return ServiceResponse.Error("Finance account transaction  not found for uuid.");
