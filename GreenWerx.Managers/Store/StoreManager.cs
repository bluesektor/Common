...
namespace GreenWerx.Managers.Store
{
    public class StoreManager : BaseManager
    {
        ...
        public ServiceResult AddCartItem(string cartUUID, string cartItemUUID, float quantity)
        {
            ...
            var res = inventoryManager.Get(cartItem.ItemUUID);
            if (res.Code != 200)
                return res;

            InventoryItem item = (InventoryItem)res.Result;
            ...
            using (var dbContext = new GreenWerxDbContext(this._connectionKey))
            {
                try
                {
        ...
        /// <param name="source">The source.</param>
        /// <returns></returns>
        public List<ShippingMethod> GetShippingMethods(ref DataFilter filter)
        {
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                return context.GetAll<ShippingMethod>(ref filter).OrderBy(sm => sm.Price).ToList();
            }
        }
        ...
        public ServiceResult ProcessPayment(CartView cart, string ipAddress)
        {
            ...
            try
            {
                ...
                {
                    if (user == null)
                    {  //use form.email to see if there is already an account. if not create one.
                    ...
                    #endregion todo Currency conversion. May not be needed or move to somewhere else.
