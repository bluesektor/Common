...
namespace GreenWerx.Managers.Geo
{
    public class LocationManager : BaseManager, ICrud
    {
        ...
        public ServiceResult Get(string uuid)
        {
            if (string.IsNullOrWhiteSpace(uuid))
                return ServiceResponse.Error("UUID was not provided.");
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                ///if (!this.DataAccessAuthorized(p, "GET", false))return ServiceResponse.Error("You are not authorized this action.");
                var res = context.GetAll<Location>()?.FirstOrDefault(sw => sw.UUID == uuid);
                if (res == null)
                    return ServiceResponse.Error("Location  not found for uuid.");
                return ServiceResponse.OK("", res);
            }
        }

        public ServiceResult Get(string uuid, string locationType)
        {
            if (string.IsNullOrWhiteSpace(uuid))
                  return ServiceResponse.Error("UUID was not provided."); ;
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                ///if (!this.DataAccessAuthorized(p, "GET", false))return ServiceResponse.Error("You are not authorized this action.");
                var res = context.GetAll<Location>()?.FirstOrDefault(sw => sw.UUID == uuid && sw.LocationType.EqualsIgnoreCase(locationType));
                if (res == null)
                    return ServiceResponse.Error("Location  not found for uuid & type.");
                return ServiceResponse.OK("", res);
            }
        }
        ...
        public ServiceResult Save(Location geo)
        {
            ...
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                ...
                {
                    var res = this.Get(geo.UUID);
                    if (res.Code != 200)
                        return res;

                    dbLocation = (Location)res.Result;
                }
        ...
        public Location Search(float ipNumber, float ipVersion)
        {
            ...
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                ...
                return context.GetAll<Location>()?.FirstOrDefault(w => w.IpNumStart.ConvertTo<float>() <= ipNumber && w.IpNumEnd.ConvertTo<float>() >= ipNumber && w.IpVersion == ipVersion);
            }
        }
        ...
        public List<dynamic> SearchByType(string searchText, string uuidType, bool getAll)
        {
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                switch (uuidType)
                {
                    case "EVENTLOCATION":
                        if (getAll)
                        {
                            var res = context.GetAll<EventLocation>().Where(w => w.UUID == searchText ||
                               w.Name.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ||
                               w.Address1.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ||
                               w.City.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ||
                               w.Postal.Contains(searchText, StringComparison.InvariantCultureIgnoreCase));
                            return res?.Cast<dynamic>()?.ToList();
                        }
                        else
                        {
                            var res = context.GetAll<EventLocation>().Where(w => w.UUID == searchText ||
                                ...
                                w.Address1.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ||
                                w.City.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ||
                                w.Postal.Contains(searchText, StringComparison.InvariantCultureIgnoreCase)).Take(10);
                            return res?.Cast<dynamic>()?.ToList();
                        }
                    ...
                    default:
                        if (getAll)
                        {
                            var resLoc = context.GetAll<Location>().Where(w => w.UUID == searchText ||
                                (w.Name?.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ?? false) ||
                                (w.Address1?.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ?? false) ||
                               (w.City?.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ?? false) ||
                               (w.Postal?.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ?? false));
                            return resLoc?.Cast<dynamic>()?.ToList();
                        }
                        else
                        {
                            var resLoc = context.GetAll<Location>().Where(w => w.UUID == searchText ||
                               ...
                               (w.Address1?.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ?? false) ||
                              (w.City?.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ?? false) ||
                              (w.Postal?.Contains(searchText, StringComparison.InvariantCultureIgnoreCase) ?? false)).Take(10);
                            return resLoc?.Cast<dynamic>()?.ToList();
                        }
