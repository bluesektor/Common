...
using GreenWerx.Managers.Interfaces;
...
namespace GreenWerx.Managers.Plant
{
    public class StrainManager : BaseManager, ICrud
    {
        ...
        public ServiceResult Get(string uuid )
        {
            ...
            using (var context = new GreenWerxDbContext(this._connectionKey))
            {
                var res = context.GetAll<Strain>()?.FirstOrDefault(sw => sw.UUID == uuid);
                if (res == null)
                    return ServiceResponse.Error("Strain not found for uuid.");
